  ...
  async cancelPayment(identity: Types.Classes.CUser, input: any) {
    ...
    if (!object?.id) {
      const error = new Utils.iKomidaError(
        Utils.iKomidaError.IKOMIDA_PAYMENTS_SERVICE_CANCEL_PAYMENT_VENDOR_PAYMENT_SETTINGS
      )
      return error.logAndReturn(this.logger)
    }
    ...
    if (!userPaymentModels || userPaymentModels.length !== 1) {
      const error = new Utils.iKomidaError(
        Utils.iKomidaError.IKOMIDA_PAYMENTS_SERVICE_CANCEL_PAYMENT_INVALID_USER_PAYMENT
      )
      return error.logAndReturn(this.logger)
    }
