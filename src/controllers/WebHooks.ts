...
export default class WebHooks {
  ...
  async pagseguro(contractID: string, input: any, authenticityToken?: string) {
    try {
      ...
      if (!contractModel) {
        const error = new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_PAYMENTS_SERVICE_PAGSEGURO_WEBHOOK_INVALID_CONTRACT);
      ...
      if (paymentObject) {
        ...
        if (!userPaymentModels || userPaymentModels.length !== 1) {
          const error = new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_PAYMENTS_SERVICE_PAGSEGURO_WEBHOOK_INVALID_USER);
          error.log(this.logger);
          return false;
        }
  ...
  async asaas(object: { payment: any }, authenticityToken: string | undefined) {
    try {
      ...
      if (!subscriptionResponse?.success) {
        const error = new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_PAYMENTS_SERVICE_ASAAS_WEBHOOK_CANT_GET_SUBSCRIPTION);
        error.log(this.logger);
        return false;
      }
  ...
  async cancelPayment(paymentObject: Types.Classes.Pagseguro.CPagSeguroChargeResponse, vendorPaymentGateway: DBModels.VendorPaymentGatewayModel | undefined) {
    const paymentStatus = paymentObject.status;
    ...
    const cancelPaymentObject: Types.Classes.Pagseguro.CPagSeguroCreateCharge = Types.Classes.Pagseguro.CPagSeguroCreateCharge.fromObject({
      id: paymentObject.id,
