...
app.put('/payment/:id', async (req, res) => {
  const payload = await user.updatePaymentMethod(
...
app.delete('/payment/:id', async (req, res) => {
  const payload = await user.removePaymentMethod(
    Types.Classes.CUser.fromObject(req.headers?.identity),
    req?.params?.id,
  );
  res.status(payload?.success ? 201 : 200).sendResponse(payload);
...
app.delete('/coupon/:id', async (req, res) => {
  let payload;
  if (
    BackendTypes.Roles.valueOf((Types.Classes.CUser.fromObject(req.headers?.identity)).role) ===
